...
namespace Valigator.Core
{
		...
		where TValueValidatorTwo : IValueValidator<TValue>
	{
		...
		public NullableDataValidator(TStateValidator stateValidator, TValueValidatorOne valueValidatorOne, TValueValidatorTwo valueValidatorTwo, Mapping<TSource, TValue> mapper)
		{
		...
		public Result<Option<TSource>, ValidationError[]> Validate(object model, bool isSet, Option<TSource> value)
		{
			if (_stateValidator.Validate(model, isSet, value).TryGetValue(out var success, out var failure))
			{
				...
				return ValidatorHelpers.Validate(success, some, _valueValidatorOne, _valueValidatorTwo, _mapper);
			}
