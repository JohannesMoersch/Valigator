...
namespace Valigator.Core
{
	...
	public static class ValidationDataExtensions
	{
		...
		public static ValidationData<IReadOnlyList<TValue>> ForEach<TValue>(this ValidationData<IReadOnlyList<TValue>> data, Func<ValidatorValidationData<TValue>, IInvertableValidator<TValue>> selector)
			=> data.WithValidator(new ForEachValidator<TValue>(selector.Invoke(new ValidatorValidationData<TValue>())));

		public static ValidationData<IReadOnlyList<Option<TValue>>> ForEach<TValue>(this ValidationData<IReadOnlyList<Option<TValue>>> data, Func<ValidatorValidationData<TValue>, IInvertableValidator<TValue>> selector)
			=> data.WithValidator(new NullableForEachValidator<TValue>(selector.Invoke(new ValidatorValidationData<TValue>())));
	}
}
