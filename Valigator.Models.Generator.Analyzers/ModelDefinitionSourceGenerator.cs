...
namespace Valigator.Models.Generator.Analyzers
{
	...
	public sealed class ModelDefinitionSourceGenerator : ISourceGenerator
	{
		...
		public void Execute(GeneratorExecutionContext context)
		{
			...
			{
				GetTypes(context, out var generateModelAttributeType, out var generateModelDefaultsAttributeType, out var propertyAttributeType, out var optionalType);
				...
				foreach (var candidate in receiver.Candidates)
				{
					...
					if (typeSymbol != null && typeSymbol.TryGetAttribute(generateModelAttributeType, out var generateModelAttribute) && candidate.IsPartial() && !candidate.HasBaseClass(semanticModel, context.CancellationToken))
					{
						...
						)
						{
							...
							context.AddSource(GetUniqueFileName(modelName, usedNames), CodeGenerator.GenerateModel(semanticModel, typeSymbol, generateModelAttribute, generateModelDefaultsAttributeType, propertyAttributeType, optionalType, modelNamespaceParts, modelParentClasses, modelName, context.CancellationToken));
						}
						else
		...
		private static void GetTypes(GeneratorExecutionContext context, out INamedTypeSymbol generateModelAttributeType, out INamedTypeSymbol generateModelDefaultsAttributeType, out INamedTypeSymbol propertyAttributeType, out INamedTypeSymbol optionalType)
		{
